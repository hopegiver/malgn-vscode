...
// 실행할 수 있는 경우, "manual"은 앱이 대신 실행하지 않고 안내만 하는 경우(예:
// git처럼 이 앱이 직접 설치/업데이트를 실행하지 않는 도구), "none"은 설치 경로
// 자체를 찾지 못해 아무 것도 할 수 없는 경우다. "run"인 도구는 개별 실행 시
// preview_dev_tool_update로 무엇이 바뀔지 화면에 보여주고 사용자 확인을 받은
// 뒤에만 그 계획(planId)으로 실행한다(devTools.ts handleConfirmRun). "전체
// 업데이트"는 버튼 클릭 자체를 일괄 동의로 보고 각 도구의 미리보기를 화면
// 노출·개별 확인 없이 순차 실행하되,
