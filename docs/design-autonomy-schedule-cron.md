...
**따라서 세 모드의 규칙은 "FixedTime·Hourly = 밀어서 실행 / Cron = 건너뜀"으로 정리되고, 이 비대칭의 판정 기준은 모드 이름이 아니라 "밀린 인스턴트가 그 표현이 약속한 시각 집합 안에 있는가"다.** 이 문장을 `pick()` 주석 옆에 그대로 남긴다(다음 모드를 추가하는 사람이 같은 기준으로 판정할 수 있도록).
