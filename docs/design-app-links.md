...
| **macOS — 이 PC 실측** | `ls ~/Library/WebKit/` | `com.malgn.vscode-mockup`, `tauri-app` 디렉터리가 **이미 존재**한다. 앞의 것은 identifier 변경 전(`com.malgn.vscode-mockup`) 실행 시 생성된 디렉터리로, 현재 `src-tauri/tauri.conf.json:6`의 `identifier`(`com.malgnsoft.agent`)와는 더 이상 일치하지 않는다 — 즉 **이 앱 전용 WebKit 데이터 저장소가 번들 ID 단위로 만들어짐을 보여주는 사례**다. 브라우저와 공유되는 저장소가 아니다. |
