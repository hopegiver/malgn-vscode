...
// 회귀 수습(hub 이슈 01m33qe0zhn55mczhcdgec2b61, v0.2.11): "앱에서 로그인"
// 경로(claude_auth.rs의 start_claude_auth_login)는 실사용 환경에서 claude
// CLI가 브라우저에 띄우는 인증 코드 입력 프롬프트를 이 앱이 채울 방법이
// 없어(stdin=/dev/null) 사용자를 멈춘 화면에 가둔다. 백엔드(claude_auth.rs,
// lib.rs 커맨드 등록)는 나중에 고쳐 되살릴 코드라 그대로 남겨두고, 이 화면의
// 진입점(handleClaudeAuthLoginStart/Cancel, "앱에서 로그인" 버튼,
// state.claudeAuthLogin 참조)만 호출을 끊는다. 되살릴 때는 이 커밋을 되돌려
// 그 함수들과 버튼 분기를 복원하면 된다 — 검증된 "터미널 열기" 경로만
// 남긴다.
