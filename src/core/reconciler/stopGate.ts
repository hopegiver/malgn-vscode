// STOPPABLE_SURFACES + 정지 판정 — architecture.md §3.6.1(정본, 6경로×2표면×install 매트릭스)·
// §3.5.3·§2.1·§3.7.3(N-2) 구현. v1.2-stopmatrix — architect 판정에 따라 install 예외 범위를
// 축소한다. v2.0-native(W-N2·W-N3) — ③을 대상 폴더 신뢰 원장(C-11 해소)으로, ⑥ 무인 세션
// 게이트(T-3 신설)를 매트릭스에 추가했다.
//
...
// (§3.5.3) / 대상 폴더 미신뢰(§3.6.1 ③, 신뢰 원장) / 정책 체크아웃 14일 노후
// (§3.7.3·N-2) / 고위험 표면 변경 재동의(HRS 4, §7.3.1) / 무인 세션(§3.6.1 ⑥) 여섯
// 경로가 전부 `evaluateStop()` 하나를 통해서만 정지를 표현한다. HRS 4의 "표면 파일 변경
// 감지" 자체(무엇이 바뀌었는지 판단)는 W7 소관이고, 대상 폴더 신뢰 원장의 조회
// (`core/trust/ledger.ts`)와 무인 세션 판정(OS별 세션 attach 조회)의 *계산*도 이 모듈의
// 책임이 아니다 — 이 모듈은 그 판정 결과(불리언 신호)가 이 상수를 통해서만 정지로
// 이어지도록 배선만 제공한다.
//
...
// 반환으로 install을 정지 경로 **전부**에서 배제했다. 그러나 그 근거(§3.5.3 "하한
...
//   ③ 대상 폴더 미신뢰 — install도 정지(예외 없음, §3.6.1 판정 A-1 — 미신뢰 대상 폴더는
...
//   ⑥ 무인 세션(신설) — install도 정지(예외 없음 — §3.6.1 ⑥ "정지(예외 없음)". 무인
//                          상태에서 install의 자동 실행을 허용하면 T-3가 막으려던
//                          "되돌릴 수 없는 전역 설치가 사람 없이 도는 것"이 그대로 열린다)
...
// 여기서는 "복구 경로 축"에 적용한 것). 다른 다섯 경로(킬 스위치·호환 게이트·대상 폴더
// 미신뢰·정책 노후·무인 세션)는 두 표면을 함께 정지시킨다 — N-2 표(§2.5)가 "apply()와
// 신규 동의 발급만 멈추고"라고 명시한 것과 §3.6.1이 이전 세 표에 같은 어휘가
// "복붙"돼 있었다고 서술한 것으로 보아 다섯 경로가 원래 동일한 두 표면 벡터를 썼다고
...
/** [개명 · C-11 해소] 구 `MV_STOP_UNTRUSTED_WORKSPACE` — "미신뢰 워크스페이스" 개념 자체가
 * 네이티브에 없다(§3.6.1 ③ "미신뢰 워크스페이스 → 신뢰 원장에 없거나 untrusted인 대상
 * 폴더"). 신호원은 `core/trust/ledger.ts`(TrustLedger, 기본값 untrusted). */
export const MV_STOP_TARGET_FOLDER_UNTRUSTED = 'MV_STOP_TARGET_FOLDER_UNTRUSTED';
...
/** [신설 · W-N3] §3.6.1 ⑥ 무인 세션 게이트(T-3) — "판정 불가 시 fail-closed"
 * (§3.6.1 ⑥ "포기·감당": `sessionAttended = false`). */
export const MV_STOP_UNATTENDED_SESSION = 'MV_STOP_UNATTENDED_SESSION';

export type StopReasonCode =
  | typeof MV_STOP_KILL_SWITCH
  | typeof MV_STOP_COMPAT_GATE_BELOW_MINIMUM
  | typeof MV_STOP_TARGET_FOLDER_UNTRUSTED
  | typeof MV_STOP_POLICY_CHECKOUT_STALE
  | typeof MV_STOP_HRS4_RECONSENT_REQUIRED
  | typeof MV_STOP_UNATTENDED_SESSION;
...
// ---------------------------------------------------------------------------
// 입력 신호 — 6개 정지 경로 각각의 "이미 계산된" 판정 결과.
...
export interface StopSignals {
  ...
  /** [개명 · C-11 해소] §3.6.1 ③ — 대상 폴더 신뢰 원장(`core/trust/ledger.ts`,
   * `TrustLedger.get()`) 조회 결과가 `'trusted'`인가. **기본값 `untrusted`**(원장에 없는
   * 폴더·원장 파일 자체가 없는 최초 실행 포함) — 이 필드에 상수 `true`를 대입하면 C-11이
   * 그대로 재현된다(구 `workspaceTrusted`가 겪은 실패). 호출자는 반드시 원장 조회
   * 결과를 넘겨야 한다(`ledger.test.ts`가 "원장 제거 시 이 필드가 상수화되면 회귀
   * 테스트가 실패한다"를 별도로 증명한다). */
  ...
  /** [신설 · W-N3] §3.6.1 ⑥ — 사람이 지금 이 세션 앞에 있는가(세션 attach·화면 잠금·
   * 콘솔 vs 원격 세션·앱 창 포그라운드 판정 결과). **판정 불가 시 `false`**(fail-closed,
   * §3.6.1 ⑥ "포기·감당"). 이 신호의 실제 OS별 계산은 이 모듈의 책임이 아니다 — 여기서는
   * "이미 계산된 결과가 이 상수를 통해서만 정지로 번역되는" 배선만 제공한다. */
  readonly sessionAttended: boolean;
}
 ...
 * `providerId === 'install'`을 별도로 재확인한다(방어심층) — `minAppVersion`/
 * `maxAppVersion`(구 `min/maxExtensionVersion`, §3.6.1 M-9 개명. 버전 상하한, "provider
 * 전체" 적용)도 함께 면제한다: 버전 상하한이
...
function killSwitchStops(
  ...
  if (killSwitch.minAppVersion) {
    const min = parseVersion(killSwitch.minAppVersion);
    if (min && compareVersions(current, min) < 0) return true;
  }
  if (killSwitch.maxAppVersion) {
    const max = parseVersion(killSwitch.maxAppVersion);
 ...
 * **③ 대상 폴더 미신뢰·④ 정책 체크아웃 노후·⑥ 무인 세션은 install에도 예외 없이
 * 적용된다** — 이 세 상태는 install의 `apply()`가 해소할 수 있는 상태가 아니기
 * 때문이다(규칙 A가 성립하지 않는다). 블랭킷 이른 반환을 두지 않고 신호마다 개별
 * 판정하는 것이 이 축소의 핵심이다 — 진입부 한 곳에서 install을 걸러내면 ③④⑥이
 * 구조적으로 함께 면제돼 버려 규칙 A가 요구하는 "상태별로 다르게" 판단을 표현할 수
 * 없다.
 */
export function evaluateStop(
  ...
  // ③ 대상 폴더 미신뢰(§3.6.1 ③, C-11 해소) — provider.apply·consent.issue 둘 다
  //    정지. install도 예외 없이 정지된다(§3.6.1 판정 A-1 — 미신뢰 대상 폴더는 install이
  //    해소할 수 있는 상태가 아니고, policy-contract.md §2.2 T-4가 I-B 매니저 경로
  //    화이트리스트의 대체물로 이 원장을 명문화했다). `targetFolderTrusted`는 신뢰
  //    원장(`core/trust/ledger.ts`) 조회 결과이고 **기본값 untrusted**다 — 이 필드가
  //    상수 `true`로 굳으면(C-11의 실질) 이 조건은 영원히 통과하지 못하는 죽은 코드가
  //    된다는 것이 이 신호의 "주입 불가" 근거다.
  if (!signals.targetFolderTrusted) {
    reasons.push({
      code: MV_STOP_TARGET_FOLDER_UNTRUSTED,
      message: '신뢰되지 않은 대상 폴더에서는 적용할 수 없습니다 — 이 폴더를 신뢰하면 진행할 수 있습니다',
    });
  }
  ...
  // ⑥ 무인 세션(§3.6.1 ⑥, 신설·W-N3·T-3) — provider.apply·consent.issue 둘 다
  //    정지한다. install도 예외 없이 정지된다(규칙 A 미성립 — 무인 상태는 install의
  //    apply()가 해소할 수 있는 상태가 아니다. §3.6.1 ⑥ 표: "정지 | 정지 | 정지(예외
  //    없음)"). `sessionAttended`는 이 함수 밖에서 절대 읽히지 않는다(AT-S1,
  //    `stopGateArchitecture.test.ts`) — "무인이면 아무 것도 안 한다"를 재수렴 루프
  //    진입부의 이른 반환으로 전역화하지 않는다는 것이 §3.6.1 ⑥의 요점이다: 이 신호는
  //    STOPPABLE_SURFACES에 적힌 두 표면만 정지시키는 매트릭스의 한 행일 뿐이고,
  //    장래 표면(예: `worker.claim`)이 추가되면 그 표면에 대해 규칙 A/B를 새로
  //    판정한다.
  if (!signals.sessionAttended) {
    reasons.push({
      code: MV_STOP_UNATTENDED_SESSION,
      message: '적용은 Malgn 창을 열어 진행합니다',
    });
  }
