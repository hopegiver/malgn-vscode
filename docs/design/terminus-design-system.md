...
| 요소 유형 | 폰트 |
|---|---|
| 한글이 섞일 가능성이 있는 모든 UI 텍스트(탭/사이드바/버튼/라벨/본문/인사말/경로(한글 폴더명 가능)/CLI 로그/Rust가 만든 명령 안내 문구/한글 단위가 붙은 값 등) | `--font-display` / `--font-body` |
| 한글이 절대 섞이지 않는 순수 숫자·버전·해시/ID·시계 leaf 요소(예: 세션ID `#a591d9dc…`, 버전 `v0.2.8`) | `--font-numeric` |
...
**정정(배치 B, 2026-09-24)**: 위 1번의 "사용량(최근 활동일)"과 3번의 "사용량
기간 토글(7일/30일)"은 PM 결정으로 폐기됐다 — 사용량 사이드바는 이제 정적
nav 행("일별 사용량") 1개뿐이라 `.ws-row`/`.sidebar-nav-row.current` 둘 다
이 탭에서는 쓰이지 않는다(다른 5개 사용처는 그대로 유효). 상세는
`terminus-shell-ia.md` §4-4 참고.
